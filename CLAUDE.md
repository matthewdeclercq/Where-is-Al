...
1. `tagPointsOnOffTrail()` — Uses `at-trail-simplified.js` + `trail-proximity.js` to tag each point as on/off-trail (0.25mi threshold)
2. `snapPointsToTrail()` — Uses `at-trail-with-miles.js` + `trail-distance.js` to snap on-trail points, adding `trailMile` and `trailElevation`
...
| `trail-distance.js` | `snapToTrail()`, `snapPointsToTrail()` for trail-mile snapping |
